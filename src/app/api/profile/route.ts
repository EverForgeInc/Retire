...
function serializeProfile(profile: NonNullable<Awaited<ReturnType<typeof getSessionProfile>>>["profile"]) {
  ...
  return {
    ...
    intendedPostServiceCountry: profile.intendedPostServiceCountry,
    moving: profile.moving,
    spouse: profile.spouse,
    dependents: profile.dependents,
    skillbridgeIntent: profile.skillbridgeIntent,
    ...
    vaClaimIntent: profile.vaClaimIntent,
    vaHealthCareIntent: profile.vaHealthCareIntent,
    civilianEmployment: profile.civilianEmployment,
    federalEmployment: profile.federalEmployment,
...
export async function PUT(request: Request) {
  try {
    ...
    const payload = {
      ...
      intendedPostServiceCountry: data.intendedPostServiceCountry,
      moving: data.moving ?? false,
      spouse: data.spouse ?? false,
      dependents: data.dependents ?? 0,
      skillbridgeIntent: data.skillbridgeIntent,
      ...
      vaClaimIntent: data.vaClaimIntent,
      vaHealthCareIntent: data.vaHealthCareIntent,
      civilianEmployment: data.civilianEmployment,
      federalEmployment: data.federalEmployment,
      ceremony: data.ceremony,
      terminalLeaveStart: data.terminalLeaveStart ? parseDateOnly(data.terminalLeaveStart) : null,
